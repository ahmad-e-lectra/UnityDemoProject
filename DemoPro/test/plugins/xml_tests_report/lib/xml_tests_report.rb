  ...
  def write_failures(results, stream)
    ...
    results.each do |result|
      result[:collection].each do |item|
        # --- FIX STARTS HERE ---
        # Safely access path and file, defaulting to empty string if nil
        path = result[:source][:path] || ''
        file = result[:source][:file] || 'unknown_file.c'
  ...
  def write_tests(results, stream, tag)
    ...
    results.each do |result|
      result[:collection].each do |item|
        ...
        stream.puts "\t\t<Test id=\"#{@test_counter}\">"
        stream.puts "\t\t\t<Name>#{filename}::#{item[:test]}</Name>" # Use the corrected filename
        stream.puts "\t\t</Test>"
        @test_counter += 1
      end
    end
